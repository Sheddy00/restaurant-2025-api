    ...
    public List<Price> saveAll(List<Price> entities) {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            entities.forEach(entityToSave -> {
                try {
                    Long id = entityToSave.getId() == null ? nextValId.nextID("price", connection) : entityToSave.getId();
                    statement.setLong(1, id);
                    statement.setDouble(2, entityToSave.getAmount());
                    statement.setDate(3, Date.valueOf(entityToSave.getDateValue()));
                    statement.setLong(4, entityToSave.getIngredient() != null ? entityToSave.getIngredient().getId() : 0);
